...
def define_A(input_nc, output_nc, ngf, n_blocks, n_blocks_shared, n_domains, norm='batch', use_dropout=False, gpu_ids=[]):
    flag = 1
    ...
    if n_blocks_shared > 0:
        ...
        plex_netA = G_Plexer(n_domains, ResnetGenEncoder, enc_args, ResnetGenDecoder, dec_args, flag, ResnetGenShared,
                             shenc_args, shdec_args)
    else:
        plex_netA = G_Plexer(n_domains, ResnetGenEncoder, enc_args, ResnetGenDecoder, dec_args, flag)

    if len(gpu_ids) > 0:
        assert (torch.cuda.is_available())
        plex_netA.cuda(gpu_ids[0])

    plex_netA.apply(weights_init)
    return plex_netA

def define_B(input_nc, output_nc, ngf, n_blocks, n_blocks_shared, n_domains, norm='batch', use_dropout=False, gpu_ids=[]):
    flag = 0
    norm_layer = get_norm_layer(norm_type=norm)
    if type(norm_layer) == functools.partial:
        use_bias = norm_layer.func == nn.InstanceNorm2d
    ...
    if n_blocks_shared > 0:
        ...
        shenc_args = (n_domains, n_blocks_shenc) + dup_args
        shdec_args = (n_domains, n_blocks_shdec) + dup_args
        plex_netB = G_Plexer(n_domains, ResnetGenEncoder, enc_args, ResnetGenDecoder, dec_args, flag, ResnetGenShared, shenc_args, shdec_args)
    else:
        plex_netB = G_Plexer(n_domains, ResnetGenEncoder, enc_args, ResnetGenDecoder, dec_args, flag)

    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        plex_netB.cuda(gpu_ids[0])

    plex_netB.apply(weights_init)
    return plex_netB
...
class G_Plexer(Plexer):
    def __init__(self, n_domains, encoder, enc_args, decoder, dec_args, flag,
