...
class UnalignedDataset(BaseDataset):
    def __init__(self, opt):
        ...
        totalindex = numpy.zeros((1, sum(self.sizes)))
    ...
    def load_image(self, dom, idx):
        ...
        #old_img = Image.open(path).convert('L')
    ...
    def __getitem__(self, index):
        if not self.opt.isTrain:
            ...
            while True:
                while True:
                    DA = index % len(self.dirs)
                    DB = random.randint(0, len(self.dirs) - 1)
                    #DB = random.sample(range(len(self.dirs)), 1)
                    if not DA == DB:
                        break
                index_A = random.randint(0, self.sizes[DA] - 1)
                index_B = random.randint(0, self.sizes[DB] - 1)
                if self.index[0, sum(self.sizes[0:DA]) + index_A] == 0 and self.index[0, sum(self.sizes[0:DB]) + index_B] == 0:
                    self.index[0, sum(self.sizes[0:DA]) + index_A] = 1
                    self.index[0, sum(self.sizes[0:DB]) + index_B] = 1
                    break
        ...
        if index == len(self.dirs)*200 - 1:
           self.index = numpy.zeros((1, sum(self.sizes)))
        return bundle

    def __len__(self):
        if self.opt.isTrain:
            #return min(self.sizes)
            return len(self.dirs)*200
